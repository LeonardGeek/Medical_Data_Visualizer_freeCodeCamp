...
def draw_cat_plot():
    ...
    df_cat = df_cat = df_cat.reset_index().groupby(['variable', 'cardio','value']).count().reset_index().rename(columns={'index': 'total'})
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, annot=True,linewidths=2, fmt='.1f', ax=ax)
